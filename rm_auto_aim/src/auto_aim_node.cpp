...
namespace rm_auto_aim
{
    AutoAimNode::AutoAimNode(const rclcpp::NodeOptions &options)
    {
        ...
        cmd_gimbal_pub_ = node_->create_publisher<rm_interfaces::msg::GimbalCmd>(
                "/cmd_gimbal",
                10);
    ...
    void AutoAimNode::ProcessImage(cv::Mat &img, double time_stamp_ms)
    {
        ...
        if(target.size() > 0)
        {
            cv::Point2f center = cv::Point2f(img.rows/2, img.cols/2);
            float pitch = (center.y - target[0].point[0].y) / center.y * 45;
            float yaw = (target[0].point[0].x - center.x) / center.x * 45;
            rm_interfaces::msg::GimbalCmd Gimbal_msg;
            Gimbal_msg.position.pitch = pitch;
            Gimbal_msg.position.yaw = yaw;
            cmd_gimbal_pub_->publish(Gimbal_msg);
        }
#endif
